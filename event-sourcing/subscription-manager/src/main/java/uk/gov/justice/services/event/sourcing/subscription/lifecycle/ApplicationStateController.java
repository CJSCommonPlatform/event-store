...
import uk.gov.justice.services.event.sourcing.subscription.lifecycle.catchup.CatchupListener;
import uk.gov.justice.services.event.sourcing.subscription.lifecycle.shuttering.ShutteringListener;
import uk.gov.justice.services.event.sourcing.subscription.lifecycle.shuttering.events.ObjectShutteredEvent;
import uk.gov.justice.services.event.sourcing.subscription.lifecycle.shuttering.events.ObjectUnshutteredEvent;
import uk.gov.justice.services.event.sourcing.subscription.lifecycle.shuttering.events.ShutteringCompleteEvent;
import uk.gov.justice.services.event.sourcing.subscription.lifecycle.shuttering.events.ShutteringRequestedEvent;
import uk.gov.justice.services.event.sourcing.subscription.lifecycle.shuttering.events.UnshutteringCompleteEvent;
...
/**
 * Controller for sending messages as events throughout the application. These events
 * are application events and are for the control of the application itself, as opposed
 * JsonEnvelope events, which are business events for business logic.
 */
@ApplicationScoped
public class ApplicationStateController {
    ...
    /**
     * Called by any service that wishes to initiate catchup. For example a JMX MBean.
     * Any service that need to perform some activity before catchup (for example shuttering)
     * should implement this method and add itself as a listener to this controller.
     *
     * @param catchupRequestedEvent
     */
    ...
    /**
     * Called by the catchup process to inform other parts of the system that catchup has
     * started
     * @param catchupStartedEvent
     */
    ...
    /**
     * Called by any service that wishes to initiate the shuttering of services. For example a
     * JMX MBean. Any service that need to perform some activity before shuttering
     * should implement this method and add itself as a listener to this controller.
     *
     * @param shutteringRequestedEvent
     */
    ...
    /**
     * Fired by the shuttering process to inform all listeners that all Objects are shuttered
     * and shuttering is complete
     * 
     * @param shutteringCompleteEvent
     */
    ...
    /**
     * Called by any service that wishes to initiate the unshuttering of services. For example a
     * JMX MBean. Any service that need to perform some activity before unshuttering
     * should implement this method and add itself as a listener to this controller.
     *
     * @param unshutteringRequestedEvent
     */
     ...
     * and shuttering is complete
     *
     * @param unshutteringCompleteEvent
     */
