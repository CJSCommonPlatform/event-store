...
public class ActiveStreamsProcessorTest {
    ...
    @Mock
    private LinkedEventsProcessor linkedEventsProcessors;
    ...
    public void shouldProcessActiveStreams() throws SQLException, DatasourceException {
        ...
        verify(linkedEventsProcessors).populateLinkedEvents(streamId1, eventJdbcRepository);
        verify(linkedEventsProcessors).populateLinkedEvents(streamId2, eventJdbcRepository);
        verify(linkedEventsProcessors).populateLinkedEvents(streamId3, eventJdbcRepository);
    }
}